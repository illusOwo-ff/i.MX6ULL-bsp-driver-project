...
不同于"调用子系统接口写**设备驱动**"，本项目编写的是**控制器驱动**：基于内核框架结构体（`spi_master` / `i2c_adapter` / `uart_port` / `gpio_chip` + `irq_chip`/`irq_domain`），查 i.MX6ULL 参考手册操作真实寄存器，填充硬件相关层，让 SPI / I2C / UART / GPIO 子系统在本板上运行。
...
| 项目        | 说明                                                 |
| ----------- | ---------------------------------------------------- |
| 开发板      | 100ASK i.MX6ULL Pro（韦东山）                        |
| SoC         | NXP i.MX6ULL（Cortex-A7）                            |
| 内存 / 存储 | 512MB DDR3 / 4GB eMMC                                |
| 网络        | 双 LAN8720A PHY（RMII，MDIO 地址 0 / 1）             |
| 验证外设    | ADXL345（SPI）、AP3216C（I2C）、SR501（GPIO 中断）等 |
...
> BSP 的改动以 **patch + 自定义 defconfig/dts** 形式存放（不重分发 NXP 原厂源码树），应用到对应版本的原厂源码即可复现。
...
- **SPI 控制器驱动**：基于 `spi_master` 的 `transfer_one`，实现 PIO 轮询与中断两种传输模式、模块参数运行时切换；含 ADXL345 设备驱动、应用程序、Shell + Ftrace 性能 benchmark，量化得"小数据量 PIO 占优、大数据量中断占优"的交叉点结论。
...
```bash
# 在 NXP U-Boot 2017.03 源码中应用 patches，使用提供的 defconfig
...
```bash
# 在 NXP linux-imx 4.9.88 源码中应用 patch、放入 dts 与 defconfig
...
```bash
# 在 Buildroot 中放入 configs/my_imx6ull_defconfig 与 overlay/
